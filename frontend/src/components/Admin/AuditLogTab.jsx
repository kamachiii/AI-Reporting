...
export default function AuditLogTab() {
  ...
  const statusBadge = (s) => {
    ...
    if (s === 'success') return <span className={`${base} bg-emerald-500/10 text-emerald-400`}><CheckCircle size={12} /> Sukses</span>;
    if (s === 'error' || s === 'failed') return <span className={`${base} bg-red-500/10 text-red-400`}><XCircle size={12} /> Gagal</span>;
    return <span className={`${base} bg-amber-500/10 text-amber-400`}><Clock size={12} /> {s || 'Unknown'}</span>;
  };
      ...
      <div className="flex items-center gap-2 flex-wrap bg-surface-soft/50 border border-hairline rounded-lg p-2">
        <div className="relative flex-1 min-w-[160px]">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-sm bg-canvas border border-hairline rounded-md focus:outline-none focus:ring-1 focus:ring-accent/40"
          />
        </div>
              ...
              <tbody>
                {logs.map((l) => (
                  <motion.tr
                    ...
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="border-b border-hairline/60 last:border-0 hover:bg-surface-soft/40"
                  >
